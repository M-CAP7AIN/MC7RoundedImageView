...
public class RoundedImageView extends ImageView {
  ...
  public RoundedImageView(Context context ,int rscaleType ,float rBorderWidth ,int rBorderColor ,
		  boolean rMutateBackground , boolean rIsOval ,int rtileMode , int rtileModeX , int rtileModeY) {
      super(context);
      init(null, 0 , rscaleType ,rBorderWidth, rBorderColor , rMutateBackground , rIsOval , rtileMode , rtileModeX , rtileModeY);
  }
  ...
  private void init(AttributeSet attrs, int defStyle) {
    ...
    float cornerRadiusOverride = -1;
       

    mCornerRadii[Corner.TOP_LEFT] = -1;
        
    mCornerRadii[Corner.TOP_RIGHT] =-1;
        
    mCornerRadii[Corner.BOTTOM_RIGHT] =-1;
        
    mCornerRadii[Corner.BOTTOM_LEFT] =-1;
  ...
  private void init(AttributeSet attrs, int defStyle ,int rscaleType ,float rBorderWidth ,int rBorderColor ,
		  boolean rMutateBackground , boolean rIsOval ,int rtileMode , int rtileModeX , int rtileModeY) {


    int index = rscaleType;
    if (index >= 0) {
      setScaleType(SCALE_TYPES[index]);
    } else {
      // default scaletype to FIT_CENTER
    ...
    mBorderWidth = rBorderWidth;
    if (mBorderWidth < 0) {
      mBorderWidth = DEFAULT_BORDER_WIDTH;
    }

    mBorderColor =  ColorStateList.valueOf(rBorderColor);
    if (mBorderColor == null) {
      mBorderColor = ColorStateList.valueOf(RoundedDrawable.DEFAULT_BORDER_COLOR);
    }

    mMutateBackground = rMutateBackground;
    mIsOval = rIsOval;

    final int tileMode = rtileMode;
    if (tileMode != TILE_MODE_UNDEFINED) {
      setTileModeX(parseTileMode(tileMode));
      setTileModeY(parseTileMode(tileMode));
    }

    final int tileModeX =rtileModeX;
    if (tileModeX != TILE_MODE_UNDEFINED) {
      setTileModeX(parseTileMode(tileModeX));
    }

    final int tileModeY =rtileModeY;
    if (tileModeY != TILE_MODE_UNDEFINED) {
      setTileModeY(parseTileMode(tileModeY));
    }
